...
#define func_start	print_with_color("%s >>>>> >>>>>", __FUNCTION__)
#define func_end	print_with_color("<<<<< <<<<< %s", __FUNCTION__)
...
void *_some_api_internal(void *data)
{
	register_thread_font();
	func_start;

	print_with_color("new context create in some_api()");
	...
	print_with_color("main loop run in some_api()");
	g_main_loop_run(loop);
	print_with_color("main loop exit in some_api()");
...
{
	func_start;
	print_with_color("create internal thread in some_api()");

	pthread_t th;
	pthread_create(&th, NULL, _some_api_internal, NULL);

	print_with_color("internal sleep in some_api()");
//	sleep(2);
	func_end;
}
...
int main()
{
	...
	print_with_color("main thread create context");
	GMainContext *ctx = g_main_context_new();
	GMainLoop *loop = g_main_loop_new(ctx, FALSE);
