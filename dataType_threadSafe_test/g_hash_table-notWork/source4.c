...
//		print_with_color("push data :%d", (base + i));
		g_hash_table_insert(my_queue, (base + i), NULL);
	//	if (!(i % 4))
	//		usleep(1);
	}
...
int test_run() {
	...
	my_queue = g_hash_table_new(g_direct_hash, g_direct_equal);
	...
	int len = g_hash_table_size(my_queue);
